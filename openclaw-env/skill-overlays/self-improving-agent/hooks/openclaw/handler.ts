...
type HookConfig = {
  formalCooldownMinutes?: number;
  inboxCooldownMinutes?: number;
  maxExcerptChars?: number;
  skipSubagents?: boolean;
};

type Classification = {
  target: 'learnings' | 'errors' | 'features' | 'inbox';
  confidence: 'high' | 'low';
  title: string;
  summary: string;
  detailLabel: string;
...
function getHookConfig(event: any): HookConfig {
...
function shouldSkipSession(event: any): boolean {
...
function learningsDir(workspaceDir: string): string {
...
function sanitizeExcerpt(value: string, maxChars: number): string {
...
function strongCorrection(text: string): boolean {
  return [
...
function weakCorrection(text: string): boolean {
  return [
    /(?:好像不对|有点不对|不太对|似乎不对)/,
...
function strongFeatureRequest(text: string): boolean {
  return [
...
function weakFeatureRequest(text: string): boolean {
  return [
...
function strongError(text: string): boolean {
  return [
...
function weakError(text: string): boolean {
  return [
    /(?:好像挂了|似乎有问题|不工作了|没反应)/,
...
function classifyInbound(text: string): Classification | null {
...
function classifyOutbound(context: Record<string, unknown>): Classification | null {
  const deliveryError = sanitizeExcerpt(String(context?.error || ''), MAX_EXCERPT_CHARS);
...
async function ensureLearningFiles(workspaceDir: string): Promise<void> {
...
async function loadState(workspaceDir: string): Promise<{ statePath: string; state: HookState }> {
...
async function saveState(statePath: string, state: HookState): Promise<void> {
...
function shouldWriteRecord(state: HookState, key: string, cooldownMinutes: number, now: Date): boolean {
...
function pruneState(state: HookState, now: Date): void {
...
async function nextSequence(filePath: string, prefix: string, stamp: string): Promise<string> {
  try {
...
async function appendFormalEntry(workspaceDir: string, event: any, payload: Classification, now: Date): Promise<void> {
  const dir = learningsDir(workspaceDir);
...
async function appendInboxEntry(workspaceDir: string, event: any, payload: Classification, now: Date): Promise<void> {
...
async function persistClassification(event: any, payload: Classification): Promise<void> {
...
async function handlePreprocessed(event: any): Promise<void> {
...
async function handleSent(event: any): Promise<void> {
  const payload = classifyOutbound((event?.context || {}) as Record<string, unknown>);
  if (payload) {
    await persistClassification(event, payload);
  }
}
