...
There are two hook styles in this skill:
- **Agent-local shell hooks** (Claude Code / Codex): reminder-style hooks such as `UserPromptSubmit` and `PostToolUse`
- **OpenClaw internal hooks**: event-driven hooks such as `agent:bootstrap`, `message:preprocessed`, and `message:sent`

OpenClaw is the proactive path for this workspace because it can auto-route only high-confidence signals into the right `.learnings/` file while downgrading weaker signals into `INBOX.md`.
...
## OpenClaw Internal Hook Notes

The bundled OpenClaw hook keeps the original bootstrap reminder and adds conservative automatic capture:

- explicit user correction → `LEARNINGS.md`
- explicit missing capability request → `FEATURE_REQUESTS.md`
