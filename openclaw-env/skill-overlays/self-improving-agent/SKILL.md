...
| Low-confidence / ambiguous signal | Log to `.learnings/INBOX.md` first; only promote after human or agent review |
...
- `INBOX.md` — low-confidence auto-capture queue for later review
...
For proactive low-risk capture plus the original bootstrap reminder:
...
| `.learnings/INBOX.md` | Low-confidence signals that are worth reviewing before formal promotion |
...
- High-confidence user-reported failure or assistant-surfaced technical error

**Low-confidence signals** (→ inbox entry first):
- "This seems off" / "好像不对"
- Vague missing-feature wishes without a clear capability gap
- Possible failures without explicit technical evidence
