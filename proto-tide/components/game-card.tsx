...
export function GameCard({ game, compact = false }: { game: Game; compact?: boolean }) {
  return (
    <Link
      href={`/game/${game.slug}`}
      className="glass group flex h-full flex-col overflow-hidden rounded-[28px] transition duration-300 hover:-translate-y-1 hover:border-emerald-300/28 hover:shadow-[0_24px_70px_rgba(35,211,162,0.18)]"
    >
      <div className={`${game.accentClass} relative ${compact ? 'h-44' : 'h-56'} w-full`}>
      ...
      <div className="flex flex-1 flex-col gap-4 p-5">
        <p className="text-sm leading-6 text-white/74">{game.summary}</p>
