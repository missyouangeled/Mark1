...
# Fix the script: change ASSET_DIR to parent
sftp = ssh.open_sftp()
# Read the uploaded script
with sftp.open(f"{WORK_DIR}/remote_infer2.py", "r") as f:
    content = f.read()

# Fix the ASSET_DIR path
content = content.replace(
    'ASSET_DIR = "/root/autodl-tmp/voice-lab/workspace/tmp/voice-replies/chattts-hybrid/asset"',
    'ASSET_DIR = "/root/autodl-tmp/voice-lab/workspace/tmp/voice-replies/chattts-hybrid"'
)

with sftp.open(f"{WORK_DIR}/remote_infer2.py", "w") as f:
    f.write(content)
sftp.close()
print("Fixed ASSET_DIR to parent", flush=True)
