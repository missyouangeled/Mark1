...
def _run_tests():
    ...
    print(f"  merged_groups={test1_stats['merged_groups']}, unique={test1_stats['unique_lines']}")
    ...
    print(f"  原始: {test2_stats['original_lines']} 行, tail 保留: {test2_stats['kept_tail_lines']}")
    ...
    print(f"\n[测试 3] 关键事件提取")
    print(f"  critical events: {test3_stats['critical_events']}")
    ...
    print(f"\n[测试 4] 非日志文本 passthrough")
    print(f"  is_log={test4_stats['is_log']}, mode={test4_stats['mode']}")
    ...
    print(f"  unique_lines={test6_stats['unique_lines']}, merged={test6_stats['merged_groups']}")
    ...
    print(f"\n[测试 7] 大日志 (1050 行)")
    print(f"  原始: {test7_stats['original_bytes']} bytes")
    print(f"  压缩: {test7_stats['crushed_bytes']} bytes")
    print(f"  压缩率: {test7_stats['ratio'] * 100:.1f}%")
    ...
    print(f"  关键事件段位置: {section_critical_pos}, 去重 head 段位置: {section_head_pos}")
