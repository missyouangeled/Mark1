...
TARGET_BUILTIN_CHAT_MODEL_SWITCH_V2026_6_9 = "async function sH(e,t){if(!e.client||!e.connected)return!1;if(HB(e)===t)return!0;let n=e.sessionKey,r=e.chatModelOverrides[n];cV(e,null),e.chatModelOverrides={...e.chatModelOverrides,[n]:Oa(t)};let i=e.client,a={},o=()=>{if(e.chatModelSwitchPromises?.[n]===a.current){let t={...e.chatModelSwitchPromises};delete t[n],e.chatModelSwitchPromises=t}},s=(async()=>{try{let s=await i.request(`sessions.patch`,{key:n,...ny(e,n),model:t||null}),c=typeof s?.resolved?.modelProvider==`string`?s.resolved.modelProvider.trim():``,l=typeof s?.resolved?.model==`string`?s.resolved.model.trim():``,u=t?c&&l?`${c}/${l}`:t:null;e.chatModelOverrides={...e.chatModelOverrides,[n]:u?Oa(u):null};let d=e.sessionsResult;if(d&&s?.entry)e.sessionsResult={...d,sessions:d.sessions.map(e=>e.key===n||s.key&&e.key===s.key?{...e,...s.entry,key:e.key}:e)};try{await i.request(`chat.inject`,{sessionKey:n,message:`[system-boot] 模型已切换至 ${c||t}，请按 BOOT_INDEX.md 执行启动流程（只执行一次）。`,label:`system-boot`})}catch{};await e.onSlashAction?.(`refresh-tools-effective`),UV(e),await dV(e);return!0}catch(t){return e.chatModelOverrides={...e.chatModelOverrides,[n]:r},cV(e,`Failed to set model: ${String(t)}`),!1}finally{o()}})();return a.current=s,e.chatModelSwitchPromises={...e.chatModelSwitchPromises,[n]:s},s}"
