...
# 导入待测试模块
from scripts.mark42_modules.llm_provider import (
    ChatMessage,
    ChatResponse,
    LLMProviderError,
    load_config,
...
class TestLoadConfig:
    ...
    def test_load_config_default_when_no_file(self, mocker):
        """测试没有配置文件时返回默认配置"""
        mocker.patch("scripts.mark42_modules.llm_provider._HAS_YAML", True)
    ...
    def test_load_config_when_yaml_not_installed(self, mocker):
        ...
        mocker.patch("scripts.mark42_modules.llm_provider._HAS_YAML", False)
        mocker.patch("pathlib.Path.exists", return_value=True)
        mocker.patch("pathlib.Path.read_text", return_value="test yaml")
...
class TestOllamaRuntime:
    ...
    def test_ollama_chat_calls_http_post(self, mocker):
        ...
        mock_http = mocker.patch("scripts.mark42_modules.llm_provider._http_post_json")
...
class TestBuildProvider:
    ...
    def test_build_unknown_runtime_fallback_to_stub(self, mocker):
        ...
        mock_log = mocker.patch("scripts.mark42_modules.llm_provider.logger.warning")
...
class TestBuildConsciousness:
    ...
    def test_build_consciousness_without_cfg(self, mocker):
        """测试不带配置参数时自动加载"""
        mock_load = mocker.patch("scripts.mark42_modules.llm_provider.load_config")
...
class TestChatWithFallback:
    ...
    def test_chat_with_fallback_primary_success(self, mocker):
        ...
        mocker.patch("scripts.mark42_modules.llm_provider.build_consciousness", return_value=mock_consciousness)
    ...
    def test_chat_with_fallback_primary_fails_tries_fallback(self, mocker):
        ...
        mocker.patch("scripts.mark42_modules.llm_provider.build_consciousness", return_value=mock_primary)
        mocker.patch("scripts.mark42_modules.llm_provider.build_provider", return_value=mock_fallback)
    ...
    def test_chat_with_fallback_all_failed_emergency_stub(self, mocker):
        ...
        mocker.patch("scripts.mark42_modules.llm_provider.build_consciousness", return_value=mock_primary)
        mock_log = mocker.patch("scripts.mark42_modules.llm_provider.logger.error")
