...
from mark42_modules.cluster_manager import (
    CLUSTER_DEFINITIONS,
    MAX_RESTARTS,
    ClusterConfig,
    ClusterManager,
...
def temp_state_dir():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("mark42_modules.cluster_manager.STATE_DIR", tmp_path):
            with patch("mark42_modules.cluster_manager.CLUSTERS_DIR", tmp_path / "clusters"):
                with patch("mark42_modules.cluster_manager.ACTIONS_FILE", tmp_path / "armor" / "actions.jsonl"):
                    yield tmp_path
...
def test_health_check_creates_failure_md_when_degraded(cm, temp_state_dir):
    """测试：降级时创建 FAILURE.md。"""
    with patch("mark42_modules.cluster_manager._check_contract_passed", return_value=(False, "模拟失败")):
        cm.health_check("cluster-text-compress")
...
def test_health_check_removes_failure_md_when_healthy(cm, temp_state_dir):
    ...
    # 健康检查（应该删除）
    with patch("mark42_modules.cluster_manager._check_contract_passed", return_value=(True, "")):
        cm.health_check("cluster-text-compress")
