...
def reset_logging_state():
    import mark42_modules.log_setup as log_setup
    # 重置初始化标志
...
class TestGetLogger:
    ...
    def test_get_logger_returns_logger_instance(self):
        """测试返回正确类型的 logger。"""
        from mark42_modules.log_setup import get_logger
    ...
    def test_get_logger_init_only_once(self):
        """测试初始化只执行一次。"""
        from mark42_modules.log_setup import get_logger, _init_logging, _LEVEL_MAP

        # 第一次调用
        with patch('mark42_modules.log_setup.logging.getLogger') as mock_get_logger:
            mock_root = MagicMock()
            mock_root.handlers = []
    ...
    def test_get_logger_custom_level_from_env(self):
        """测试从环境变量读取日志级别。"""
        from mark42_modules.log_setup import get_logger, _LEVEL_MAP
    ...
    def test_init_logging_existing_handlers_returns(self):
        ...
        from mark42_modules.log_setup import _init_logging
        root_logger = logging.getLogger("mark42")
        root_logger.handlers = [MagicMock()]  # 添加一个假的 handler

        with patch('mark42_modules.log_setup.logging.StreamHandler') as mock_handler:
            _init_logging()
...
class TestLogFunctions:
    ...
    def test_log_info(self):
        """测试 log_info。"""
        from mark42_modules.log_setup import log_info

        with patch('mark42_modules.log_setup.get_logger') as mock_get_logger:
    ...
    def test_log_warn(self):
        """测试 log_warn。"""
        from mark42_modules.log_setup import log_warn
    ...
    def test_log_error(self):
        """测试 log_error。"""
        from mark42_modules.log_setup import log_error
    ...
    def test_log_debug(self):
        """测试 log_debug。"""
        from mark42_modules.log_setup import log_debug
    ...
    def test_log_exception(self):
        """测试 log_exception。"""
        from mark42_modules.log_setup import log_exception
    ...
    def test_log_functions_with_no_args(self):
        """测试无参数调用。"""
        from mark42_modules.log_setup import log_info, log_warn, log_error, log_debug
...
class TestLevelMap:
    ...
    def test_level_map_contains_all_levels(self):
        ...
        from mark42_modules.log_setup import _LEVEL_MAP
...
class TestFormatConstants:
    ...
    def test_log_format_exists(self):
        """测试 _LOG_FORMAT 存在。"""
        from mark42_modules.log_setup import _LOG_FORMAT
        assert isinstance(_LOG_FORMAT, str)
        assert len(_LOG_FORMAT) > 0

    def test_date_format_exists(self):
        """测试 _DATE_FORMAT 存在。"""
        from mark42_modules.log_setup import _DATE_FORMAT
