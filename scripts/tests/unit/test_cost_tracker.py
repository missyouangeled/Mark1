...
# 导入待测试模块
from scripts.mark42_modules.cost_tracker import (
    CostRecord,
    CostTracker,
    record_cost,
    MODEL_PRICING,
...
class TestCostTracker:
    ...
    def test_record_write_failure_logged(self, tmp_path, mocker):
        """测试写入失败时记录日志，不抛异常"""
        mock_log = mocker.patch("scripts.mark42_modules.cost_tracker.logger.warning")
        test_file = tmp_path / "costs.jsonl"
...
class TestRecordCostFunction:
    ...
    def test_record_cost_creates_tracker(self, mocker):
        ...
        mock_tracker_class = mocker.patch("scripts.mark42_modules.cost_tracker.CostTracker")
