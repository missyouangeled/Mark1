...
# Handle both list output and single array
...
for idx, w in enumerate(items):
    ...
    p = os.path.join(OUT, f"smoke_min_{idx}.wav")
...
ec, out, err = run(f"cat > {WORK_DIR}/smoke_final2.py << 'SCRIPTEOF'\n{min_script}\nSCRIPTEOF\n", 10)
print(f"Write: ec={ec}", flush=True)

print("\n=== Running ===", flush=True)
ec, out, err = run(f"cd {WORK_DIR} && {PY} -u smoke_final2.py 2>&1", timeout=600)
...
# Also check if the gpu memory was used
