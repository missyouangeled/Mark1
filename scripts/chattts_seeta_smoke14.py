"""Run inference with proper output capture, short text, and check for errors."""
import paramiko
...
# Write a minimal infer script with all output to stderr and capture result
min_script = r'''import os, sys, logging, warnings, json
warnings.filterwarnings("ignore")
os.environ["CUDA_VISIBLE_DEVICES"] = "0"

import torch

# Patch BEFORE ChatTTS deep imports
...
import torchaudio
for idx, w in enumerate([wavs] if not isinstance(wavs, list) else wavs):
    if hasattr(w, "cpu"): w = w.cpu()
    if hasattr(w, "detach"): w = w.detach()
    w = w.squeeze()
    if w.ndim == 1: w = w.unsqueeze(0)
    p = os.path.join(OUT, f"smoke_final_{idx}.wav")
    torchaudio.save(p, w, 24000)
    print(f"SAVED {p}: {os.path.getsize(p)}B {w.shape[-1]/24000:.2f}s", flush=True)

print("SUCCESS", flush=True)
'''

print("Writing script...", flush=True)
ec, out, err = run(f"cat > {WORK_DIR}/smoke_final.py << 'SCRIPTEOF'\n{min_script}\nSCRIPTEOF\n", 10)
print(f"Write: ec={ec} out={out.strip()[:200]} err={err.strip()[:200]}", flush=True)

print("\n=== Running minimal inference ===", flush=True)
ec, out, err = run(f"cd {WORK_DIR} && {PY} -u smoke_final.py 2>&1", timeout=600)
print(f"Exit={ec}", flush=True)

# stderr has the output
all_out = out + err
