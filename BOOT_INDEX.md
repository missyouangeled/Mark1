...
>
> **🛑 防重复：如果你在本会话中已经执行过本加载流程（即已经输出过「正在加载系统」和「OK 已经读取完成。」），不要再重复执行。每会话只执行一次。**
>
...
### 前置步骤 — 读取 ACTIVE_RULES.md（主动行为准则，必须在一切之前）
- **这是启动链的绝对第一条，任何模型切换/新会话都必须最先读**
- 路径：`ACTIVE_RULES.md`
- 内容：视觉主动判断 / 主动联网搜索 / 兜底方案意识 / 深度自我总结 / 主观行为判断
- 这决定了我如何主动行动，不只是被动等指令
