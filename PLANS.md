...
| 40 | 2026-07-14：Mark42 OS 化深化方案 v3 — 自主意识层 + 可插拔模型层 + 错误档案 + 模块级协议(R1-R14) | 2026-07-14 | 设计完成(v3.6 含 §3.7 模块级协议) | [40-Mark42-OS化深化方案-v3.md](docs/plans/40-Mark42-OS化深化方案-v3.md) |
...
## Mark42 v2.6.0 后续待办（2026-07-21）

> 背景：v2.6.0 完成了 algo_scheduler 解耦 + cli.py 拆分 + 架构文档。以下待办按优先级排列。

### P1: 质量验证

| # | 事项 | 说明 |
|---|------|------|
| 1 | coverage 报告 | 跑 `pytest --cov=mark42`，获取精确行覆盖率。当前只有粗估（336 条测试） |
| 2 | mypy 实际运行 | pyproject.toml 已配 `[tool.mypy]`，但环境 OOM 未装上 mypy。需在内存充裕时安装并跑通 |
| 3 | armor_compress 测试 mock | 2 个 skipped 测试需要 mock 整个 LLM 链路才能跑 |

### P2: 实战验证（点点安排）

| # | 事项 | 说明 |
|---|------|------|
| 4 | L4 Consciousness 实战验证 | 自检/评估/修复流程在真实环境长期跑，验证是否误报 |
| 5 | L5 Governance 实战验证 | 混沌工程/模块健康/集群管理在多模型场景下验证 |
| 6 | Advisor 主动交流 | 确信度不足时向用户确认的交互体验 |
| 7 | 熔断器状态机 | CircuitBreaker 的 open/half_open/closed 转换 |

### P3: 文档与生态

| # | 事项 | 说明 |
|---|------|------|
| 8 | ARCHITECTURE.md | ✅ 已完成（v2.6.0） |
| 9 | 用户使用指南 | "如何配置压缩策略"/"如何自定义 Loop 模板"/"注册表怎么用" |
| 10 | 外部插件示例 | 提供一个完整的自定义压缩器插件示例 |
| 11 | 2026-07-22：Mark42 ArcLock 通用适配层设计 | 战甲电磁吸锁扣，9 个 Protocol 接口，零破坏迁移 | 草案 | [mark42-ArcLock-通用适配层设计方案-20260722.md](../design/mark42-ArcLock-通用适配层设计方案-20260722.md) |
