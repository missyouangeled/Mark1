...
2. **Phase 2 的最小入口**
   ...
   - 当前已支持：
     ...
     - `--format text|json`
     - `notify-frontstage`
   - `contract.catalog` 当前也会公开 `queryCatalog`（各 query kind 的格式/必填参数/默认 limit），供其他机器先读契约再查询
   ...
   - `source.inspect` 还会稳定暴露 `recentDeliveryCount`，方便 handoff / 排查脚本直接判断最近事件里有没有正式投递
