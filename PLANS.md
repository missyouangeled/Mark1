...
2. **Phase 2 的最小 infos-handle 已收成 contract-first 查询层**
...
4. **当前停点与验证**
   ...
   - 这一串关键提交依次是：`2f57998` → `a862517` → `45258f1` → `e7095d8` → `4de50fb` → `2575456` → `ff07c2f` → `7fe7efd` → `3aa091e` → `eb8aa4f` → `7396ede`
   - 当前最小核验保持全绿：
...
- 仍需继续做 README / HANDOFF / PLANS 三处 contract 说明对齐，避免下周继续时口径漂移
- 是否要给 `events.recent` 再补一个**有稳定顺序**的 `latestBySourceItems[]` 仍未拍板；如继续，也只应做这类窄 contract 收口，不应扩 scope
- infos-handle 的图片 / 音频输出尚未开始
...
- **Phase 1：已基本落地**
- **Phase 2：已落最小版并收紧到可 handoff**
- **Phase 3：已开始，但当前故意停在“先收 contract，不继续扩 scope”的位置**
