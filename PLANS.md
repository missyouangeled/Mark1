...
2. **Phase 2 的最小入口**
   ...
   - 当前已支持：
     ...
     - `--format text|json`
     - `notify-frontstage`
   - `contract.catalog` 当前也会公开 `queryCatalog`（各 query kind 的格式/必填参数/默认 limit），供其他机器先读契约再查询；当前 query contract 已推进到 `queryContractVersion=10`
   - `sources.latest` 现在除了保留原始 `sourceStateSnapshots / sources` keyed snapshot 外，也会额外暴露 `count / availableSources / sourceItems[]`，把每个 source 的最近摘要收成与 `sources.catalog / source.inspect` 更一致的稳定 item shape，减少消费方继续碰 raw broker event
   ...
   - `sources.catalog` 现在还会补一层 `latestEventItem`，把最近一条 source 相关事件压平成稳定 item shape（`recordType / summary / checkedAt / reportStatus / deliveryStatus` 等），方便 handoff / 排查脚本直接消费
   - `source.inspect` 还会稳定暴露 `recentDeliveryCount`、`latestEventItem` 与 `recentEventItems[]`；消费方优先读这些 item shape，就不必直接解析 `recentEvents[]` 或 `latestDelivery` 原始对象
