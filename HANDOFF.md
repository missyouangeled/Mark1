...
## ⚡ 换模型第一步：先读 SKILL_CATALOG.md

> 换模型后先看 `SKILL_CATALOG.md`，里面按类别列出了所有 Skill 的用途和触发条件（30 个）。
> 本文件（HANDOFF.md）侧重 broker/infos-handle 等系统级续接；日常技能选哪个看 SKILL_CATALOG.md。
...
1. **Richer renderer / consumer**
   - v3 image card (`image.summary-card.v3`, cardVersion=4) 已作为默认 handler：gradient shell、per-panel 渐变色条、status sparkline、checkedAt 时间戳
   - 新增 `events.timeline` kind：树状时间线视图，含 source/recordType/时间戳
   - `DEFAULT_IMAGE_PRESET = "summary-card-v3"`，v2 仍可通过显式 preset 使用

2. **Broker 数据质量**
   - `openclaw-frontstage-recovery-watch.py`：`assistant_text_mismatch` 新增 30 秒追赶窗口 — transcript 新于 history 且仍在 catchup 窗口内时判为 pending，不再频繁误报

3. **Infos-handle 新能力**
   - 新增 `health.detail` kind：逐探针健康详情（Gateway 状态/延迟/PID、HTTP 探针、provider 探针、判定理由）

4. **收尾**
   - 全量测试通过：`test-openclaw-infos-handle.py`、`test-frontstage-broker.py`、`test-frontstage-recovery-watch.py`
   - sidecar live 链验证通过

### 版本号更新
- `QUERY_CONTRACT_VERSION`: 17 → 18
- `QUERY_KINDS` 新增: `events.timeline`, `health.detail`
- `handlerCatalog` 已用 v3: `image.summary-card.v3` (cardVersion=4)

### 关键文件变更
- `scripts/openclaw-infos-handle.py`: v3 default, events.timeline, health.detail
- `scripts/openclaw-frontstage-recovery-watch.py`: text_mismatch grace period
- `scripts/test-openclaw-infos-handle.py`: 更新默认 handler/v3 断言
...
- 截至 2026-05-19 上午的收口判断:**按本轮目标(A-E)已可视为当前主线可称"最终版"的收口状态**:`infos-handle handle` + `--request-file` + `openclaw_infos_handle_contract.py` helper 已成为正式主入口;`query` / `notify-frontstage` / broker `emit` 只再保 compat 壳;剩余未做项属于下一阶段增强,不再阻塞这轮收口。
- 2026-05-19 补充:昨晚/今天早上发生过一次临时关机;恢复后已重新核对 `openclaw status`、`test-openclaw-infos-handle.py`、`test-frontstage-broker.py`、`apply-openclaw-frontstage-broker-data.py --verify-control-ui-infos-handle-sidecar` 与 `openclaw-infos-handle-sidecar.service`,当前确认主链、sidecar、broker 视图与最小 consumer 验证都仍然正常。
...
3. **Control UI / sidecar 这条增强线也已有最小可交付落点**
   ...
   - `scripts/apply-openclaw-frontstage-broker-data.py` 现也已支持 `--verify-control-ui-infos-handle-sidecar / --require-control-ui-infos-handle-sidecar`,可直接按 live branding 注入的 `summary / contract / SSE` URL 去真探测当前 sidecar,而不只是检查静态 snapshot dock 配置是否存在
...
5. **当前验证结果保持全绿**
   ...
   - `python3 scripts/apply-openclaw-frontstage-broker-data.py --verify-control-ui-infos-handle-sidecar` → `ALL PASS`
   - `systemctl --user status openclaw-infos-handle-sidecar.service` → `active (running)`
   ...
   - 当前新增 smoke / caller 回归也已进验证:
     ...
     - `python3 scripts/apply-openclaw-frontstage-broker-data.py --verify-control-ui-infos-handle-sidecar`
     - 当前这条 sidecar live 验证不只检查 `healthz / summary / contract / SSE`,也会额外走一跳真实 `handle(format=image)` 并验证 `artifactHref -> /v1/artifacts/...` 的 SVG 取回链路
     - 当前 live 输出会额外打印 `infosHandleEventsRecentConsumer`,以及在请求 sidecar 验证时打印 `controlUiInfosHandleSidecar`
...
如果用户下周一回来要继续这条线,**固定口令 / 对接暗号就定为:`继续 broker / infos-handle`**。
...
7. 跑:
   ...
   - `python3 scripts/test-frontstage-broker.py`
   - `python3 scripts/apply-openclaw-frontstage-broker-data.py --verify-control-ui-infos-handle-sidecar`
8. 看 `systemctl --user --no-pager --full status openclaw-infos-handle-sidecar.service`
9. 再决定是继续迁 caller 到 infos-handle,还是继续扩 infos-handle 的下一层 text/json 契约
