...
### 第 11 类：自动执行链路必须有 dry-run 保护 [BUG-005]

**v2.3 #5 全面审查**：`heavy_execute` 写好脚本 + 入队,**但从不自动调用 `bash {script_path}`**。
- 设计 4.2 期望 "Heavy 战甲自动分批 + 后台执行"
- 实际: `heavy_execute` 写 `# TODO: replace with actual file operation` 占位 + 收工

**根因**：
- 重构"重型战甲"时,把 `heavy_execute` 当成"占位生成器"实现,以为后续接 daemon 真执行
- **daemon 真执行的代码从来没人写过** — 链路断了
- 没有 dry-run 测试用例,谁都没发现

**教训**：
- 任何"自动执行"链路(执行脚本 / 杀进程 / 清理 / 重启)必须**默认 dry-run** + 显式 `--execute-now` flag 才真跑
- 没有 dry-run 测试,就是**有链无验证**
- 写"生成器"代码时,必须问自己"这个函数调用的下一步是什么" — 下一步不存在,生成器也是死的
- **比 silent fail 更危险的是 silent noop** — 不报错,不执行,用户以为跑了

**预防规则**：
- 重构"重型"功能时,先列**端到端调用链**:`CLI → 主函数 → 后台执行 → 状态更新 → 通知用户`
- 每一步都要问"这步是干啥的?有谁在调?"
- 如果任何一步是 `# TODO` / `pass` / silent noop → 整条链路都是死的
- 写一个 **e2e 集成测试**,真跑真后台真状态更新,跑通了再 commit
- **默认 dry-run + 显式 execute-now** 是这次审查后加的护栏,后续所有"重型"功能照抄这个模式

### 第 12 类：自动 / 不可见行为必须有据可查 [原则-2026-06-30]

**点点原话**："怕有什么意外或者自动压缩 你又不记得了"

**根因**：
- AI 跑的时间长 / 模型换了几次后,**自动触发行为会被"忘记"**
- 默默执行 → 默默回退 → 用户发现时已经太晚
- 点点一个人,系统跑偏了没人发现 = 最后一道安全网是**有据可查**

**教训（已写进 MEMORY.md 长期记忆）**：
- 任何自动行为(compress / rotate / cleanup / restart)每次都要在文档或 daily 留痕
- 不可见副作用必告知(自动做事之前告知,做完汇报)
- 写下来 > 记着("我以为我记得" = 不可靠,"文件里有" = 可靠)
- 动作链路要可复盘

**在 mark42 落地的体现**：
- `heavy_execute` 默认 dry-run,要 `--execute-now` 显式才真跑
- broker / history / actions 轮替前必须通知(已实现:rotation 状态写入 log-rotation.json)
- armor_compress 默认 skip 阈值外,真触发要写 actions.jsonl + memory-index.json 双份
- 任何修改 systemd service / 守护进程配置前,必须读 `docs/对系统操作必须要参考的崩坏案例.md`

### 第 13 类：命名 / 规范 vs 实现 必须定期对照 [审查-2026-06-30]

**v2.3 全面审查**：
- 文档说"4 守护服务",实际是 2 daemon + 1 bootstrap + 1 watchdog
- 文档说 `armor.context.warn`,实际用 `armor.compress`
- 文档说 `heavy.task.done`,实际用 `heavy.task.finished`

**根因**：
- 文档写了之后,代码改了几次,没人回头对
- 命名 "good enough" 就上了,没经过规范审查
- **没人工审查 = 规范会偏移**

**教训**：
- 每过 1-2 周,做一次"文档 vs 代码 vs 真生产" 三方对照
- 审查不是"修问题",是"找出哪里已经不同步"
- 审查报告留档(本次: `mark42-全面审查-20260630.md`)
- 把"不对齐"的事项分级:🔴 重大 / 🟠 中等 / 🟡 小
...
| # | 检查项 | 触发场景 |
|:---:|:---|:---|
| 16 | **"自动执行"默认 dry-run,显式 flag 才真跑** | 写任何带 subprocess / 文件操作的函数 |
| 17 | **端到端调用链每一步都有实现 + 测试** | 重构"重型"功能 |
| 18 | **自动行为每次都留痕(actions / daily / broker event)** | 写 cron / daemon / auto-cleanup |
| 19 | **定期文档 vs 代码三方对照审查** | 1-2 周 1 次 |
| 20 | **"silent noop" 是最危险 bug,必须 dry-run 测试发现** | 写自动化脚本时 |

---

*本节由 2026-06-30 全面审查驱动。完整 12 个问题见 `mark42-全面审查-20260630.md`。*
