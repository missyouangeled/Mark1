    ...
    for issue in diag["issues"]:
        ...
        if key == "token_awareness":
            ...
            if cs in ("ok", "no_data"):
                ...
                logger.warning(f"     实际最大 token: {issue.get('maxTokensSeen', '?')} ({issue.get('estimatedContextPercent', '?')}% 窗口)")
        ...
        if key == "probe_quality":
            ...
            if issue.get("latestCompaction"):
                ...
                logger.info(f"     最近压缩: tokensBefore={lc.get('tokensBefore', '?')}, file={lc.get('sessionFile', '?')}")
            if issue.get("advice"):
                logger.info(f"     {issue['advice']}")
            logger.info(f"     探针问题数: {len(issue.get('probeQuestions', {}))} 类 (recall/artifact/continuation/decision)")
            logger.info("")
            continue
...
def print_apply_result(result: dict[str, Any]) -> None:
    ...
    if status == "nothing_to_do":
        ...
        logger.info(f"  重启 Gateway 后生效: openclaw gateway restart")
    elif status == "error":
