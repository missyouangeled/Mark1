...
class Consciousness:
    ...
    def assess_certainty(self, issue: dict[str, Any]) -> CertaintyAssessment:
        ...
        for rule in self.rules:
            m = rule.get("match", {})
            if issue.get("source") == m.get("source") and issue.get("category") == m.get("category"):
                return CertaintyAssessment(
    ...
    def auto_remediate(
        ...
        if cat in blacklist:
            return {"ok": False, "reason": f"黑名单拒绝：{cat}", "action": "blocked_by_blacklist"}

        # 按 issue 类型选执行函数
        executor = _REMEDIATION_EXECUTORS.get(issue.get("category"))
        if executor is None:
            return {"ok": False, "reason": f"无 auto_remediate 实现: {cat}", "action": "no_executor"}
        ...
        try:
            ...
            return {"ok": True, "dry_run": False, "action": "executed", "result": result}
        except Exception as e:
            return {"ok": False, "reason": f"执行失败: {e}", "action": "execute_failed"}

    # ── C4. 主动发起对话 ──

    def dialog(self, issue: dict[str, Any], assessment: CertaintyAssessment, to: str | None = None) -> DialogRequest:
        ...
        if assessment.certainty == "100%":
            ...
            options = [
                {"id": "approve_remediation", "label": "授权战甲自动修", "desc": "按规则表推荐方案自动修复（推荐）"},
                {"id": "ask_advisor", "label": "先问 advisor", "desc": "问外部大模型确认方案是否安全"},
                {"id": "manual_decide", "label": "我自己决定", "desc": "我手动处理 / 告诉战甲别的方案"},
            ]
        elif assessment.action == "ask_user":
            options = [
                {"id": "approve_remediation", "label": "按默认方案修", "desc": "让战甲按规则表推荐方案自动修"},
                {"id": "ask_advisor", "label": "先问 advisor", "desc": "问外部大模型确认方案是否安全"},
                {"id": "manual_decide", "label": "我自己决定", "desc": "我手动处理 / 告诉战甲别的方案"},
            ]
        elif assessment.action == "ask_advisor":
            options = [
                {"id": "ask_advisor", "label": "问 advisor", "desc": "调用外部大模型确认方案"},
                {
        ...
        return DialogRequest(
            ...
            context={**issue, "assessment": assessment.to_dict(), "archive_id": archive_id},
            question=q,
            options=options,
            severity=severity,
            to=target,
    ...
    def _render_question(self, issue: dict[str, Any], assessment: CertaintyAssessment) -> str:
        """v3 §3.3 4 类场景的话术模板。"""
        src = issue.get("source", "?")
        cat = issue.get("category", "?")
        msg = issue.get("msg", "")
        if assessment.certainty == "100%":
            return f"我检测到 {src}/{cat}：{msg}。\n我的判断：{assessment.reason}\n是否授权我按此方案自动修？"
    ...
    def handle_issue(self, issue: dict[str, Any], dry_run: bool = True) -> dict[str, Any]:
        ...
        if assess.certainty == "100%" and assess.action == "auto_remediate":
            ...
            return {"path": "C3_auto_remediate", "assessment": assess.to_dict(), "remediation": rem}
                ...
                try:
                    ...
                    if advisor_result and advisor_result.success:
                        ...
                        self._circuit_breaker.record_failure(
                            "core_2_armor_consciousness", advisor_result.fallback_reason or "unknown"
                        )
                except Exception as e:
        ...
        if advisor_result and advisor_result.success and advisor_result.verdict:
            ...
            if v.is_modify and v.is_trustworthy and ask_confident:
                rem = self.auto_remediate(issue, assess, dry_run=dry_run, override_plan=v.modified_plan)
                return {
        ...
        if advisor_result:
            ...
            result["advisor_verdict"] = advisor_result.verdict.to_dict() if advisor_result.verdict else None
            result["advisor_fallback_reason"] = advisor_result.fallback_reason
        else:
            result["advisor_attempted"] = False
        return result

    def _pick_scenario(self, issue: dict[str, Any], assessment: CertaintyAssessment) -> str:
    ...
    def _call_advisor(
        self, scenario: str, issue: dict[str, Any], assessment: CertaintyAssessment
    ) -> AdvisorResult | None:
        """调用 AdvisorClient（封装异常）."""
        if not self._advisor_client:
            return None
        if scenario == "a":
            return self._advisor_client.ask_about_uncertain_issue(issue, assessment.to_dict())
        elif scenario == "b":
            ...
            entry = {
                "id": assessment.archive_entry_id or "",
                "category": issue.get("category", ""),
                "diagnosis": issue.get("msg", ""),
                "resolution": {},
            }
    ...
    def verify_read_protocol(self, min_correct: int = 8, force: bool = False) -> dict[str, Any]:
        ...
        for q in selected:
            try:
                ...
                if is_correct:
                    correct += 1
                results.append({"id": q["id"], "correct": is_correct, "hits": hits, "total": len(keywords)})
            except Exception as e:
                results.append({"id": q["id"], "correct": False, "error": str(e)})
...
def _remediate_process_down(issue: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            ["systemctl", "--user", "is-active", f"openclaw-{source}"], capture_output=True, text=True, timeout=5
        )
...
def _remediate_embed_index_missing(issue: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        result = subprocess.run([python_bin, script_path, "--force"], capture_output=True, text=True, timeout=300)
...
def _remediate_loop_not_registered(issue: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not template:
            return {"ok": False, "action": "loop_register", "reason": "缺少 template 名，无法重新注册"}
