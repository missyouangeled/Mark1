...
def test_check_low_usage_ok(mocker):
    ...
    assert result["severity"] == "ok"
    assert result["usagePercent"] < THRESHOLD_WARN
...
def test_check_warn_band_info_severity(mocker):
    ...
    assert THRESHOLD_WARN <= result["usagePercent"] < THRESHOLD_ALERT
...
def test_check_alert_band_warn_severity(mocker):
    ...
    assert THRESHOLD_ALERT <= result["usagePercent"] < THRESHOLD_CRIT
...
def test_check_crit_band_critical_severity(mocker):
    ...
    assert result["usagePercent"] >= THRESHOLD_CRIT
...
def test_token_estimation_smart_mode_zh_heavy():
    ...
    from mark42.utils import _estimate_tokens_smart
