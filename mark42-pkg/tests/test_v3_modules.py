...
def _fresh_env():
    ...
    import importlib
    # 防御：先确保模块已 import入 sys.modules 再 reload
    # （conftest 的 autouse fixture 也会 reload，全量跑时两者可能交互导致 sys.modules 状态错位）
    import mark42.config as _cfg
    import mark42.llm_provider as _lp
    import mark42.error_archive as _ea
    import mark42.consciousness as _co
    ...
    from mark42.error_archive import (
        ErrorArchive, STATUS_RESOLVED, STATUS_AUTO_APPROVED, STATUS_REJECTED,
    )
    from mark42.consciousness import Consciousness, DETERMINISTIC_RULES, CertaintyAssessment
...
def test_v3_2_error_archive():
    ...
    def w():
        try:
            for _ in range(3):
                arc.increment_auto_count(e_c.id)
    ...
    # 12. 审计日志
    from mark42.error_archive import APPROVAL_LOG_FILE
