...
- 除纯日常聊天外，默认都应有且只应有一个 `main-supervisor-lite` 在位。
- 若已有普通任务分身，则必须同时存在且只存在一个监工分身。
- 先给前台短反馈，再决定是否把重活卸到后台；不要先闷头跑。
- 工作不阻塞前台时，可只保留监工待命，不必强开普通任务分身。
- 3 分钟无可见产出时，必须由监工主动补一次状态。
- 任务空返回 / 异常结束时，必须先由监工向前台报告，再开始检查和修复。
...
- 监工分身只看“是否还有后台任务”；无后台任务持续约 2 分钟后，可从 `1` 回到 `0`。
...
- **默认轻量监工分身**：除纯日常聊天外，只要当前轮属于排查、执行、搜索、下载、转码、生成、整理、调研、修改、验证等工作型任务，就默认保留或尽快拉起一个轻量监工分身待命；当任务可能阻塞前台时，由它承担继续推进、看护进度、必要时补位回报的职责。它的目标是兜底主会话流畅性，不是抢占主会话本身。
- **监工分身固定标识**：监工分身使用保留标签 `main-supervisor-lite`；这个标签只给监工分身使用，不能复用到普通任务分身上。
- **监工唯一标号**：监工分身额外使用宿主唯一标号 `main-supervisor-lite@<runtime-host>`；其中 `<runtime-host>` 优先取运行时 host 元数据。这样即使跨机器同步规则，也能避免把别的机器监工误当成本机监工。
- **单例规则**：监工分身整体数量必须遵循“0 或 1”的原则；工作型任务期应有且只有一个 `main-supervisor-lite`，无后台任务持续约 2 分钟后可回到 0。若发现重复，先比较宿主唯一标号；若标号相同，则优先保留最新且健康的一个（健康 = 未失败、未超时、未被杀，且近期仍有活动或可见进度），其余视为异常重复实例并收掉。
- **分身收口规则**：普通任务分身与监工分身要分开处理。日常收尾时，默认只关闭已完成、失败、超时或明显无用的任务分身，不要顺手把 `main-supervisor-lite` 一起关掉；监工分身只看“当前是否还有后台任务”，不看主会话是否仍在聊天。只要“当前没有任何后台任务”持续约 2 分钟，就允许关闭 `main-supervisor-lite`。
...
- **默认执行顺序**：1）先给主会话一个短反馈；2）若当前不是纯日常聊天，则检查 `main-supervisor-lite` 是否已在位，若没有则尽快补上，且只保留一个；3）判断当前工作会不会阻塞前台，若会则立刻卸到后台任务分身；4）前台继续正常对话，只在关键节点主动插播；5）凡是媒体相关改动，都以主会话里的真实展示/播放成功为准；6）任务收尾时默认先关闭普通任务分身；若已无任何后台任务，则开始计算监工分身的 2 分钟空闲窗口，时间到后关闭 `main-supervisor-lite`。

## main-supervisor-lite 操作模板

当前在 WebChat / Control UI 直聊里的可落地实现，不是线程绑定的持久监工会话，而是：**保留标签 + 单例语义 + 按需轻量拉起 + 无后台任务 2 分钟后收掉**。

默认按以下顺序执行：

1. **启动判断**
   - 只要当前轮不是纯日常聊天，就应保证 `main-supervisor-lite` 在位。
   - 若当前轮工作可能阻塞主会话，先给主会话一个短反馈。
   - 然后检查 `main-supervisor-lite` 是否已在位；若没有，则尽快补一个，并确保始终只保留一个。
2. **任务分流**
   - 真正做事的重活交给普通任务分身。
   - 若当前工作并不会阻塞前台，可只保留 `main-supervisor-lite` 待命，不必强开普通任务分身。
   - `main-supervisor-lite` 只负责监工、兜底、补位回报，不承担普通任务分身的常规身份。
   - **禁止孤立任务分身**：只要存在普通任务分身，就必须同时存在且只存在一个 `main-supervisor-lite`；不允许出现“只有任务分身、没有监工分身”的状态。
3. **渠道约束**
...
4. **检查顺序**
   ...
   - 普通任务分身按任务状态处理；监工分身按“是否还有后台任务”处理。
5. **收尾顺序**
   - 先关闭普通任务分身。
   - 若此时已经没有任何后台任务，则开始计算 `main-supervisor-lite` 的 2 分钟空闲窗口；期间如果又出现新后台任务，则取消这次收口，保留监工分身。
6. **异常模板**
   ...
   - **任务空返回**：把“空返回”视为异常完成，不当作成功。先由 `main-supervisor-lite` 向主会话报告“后台任务空返回/结果异常，我正在接手检查”，再检查对应任务分身状态；若任务是低风险且可重试的，可自动重试 1 次；若再次空返回，则显式报阻塞，不再静默重试。
   - **任务异常结束**：若任务分身失败、超时、被杀掉、连接中断或异常结束，必须先由 `main-supervisor-lite` 向前台报告“后台任务异常结束，我正在修复”，再开始检查、修复、必要时重开任务分身。
   - **3 分钟无可见产出**：这里的“可见产出”不是指任务必须 3 分钟内完成，而是指用户前台在 3 分钟内至少应该看到一次短反馈、进度、阶段结果或卡点说明。即使一切正常、只是还在等待，也必须由 `main-supervisor-lite` 主动补一句“仍在运行 / 还在等 / 下一次何时回报”，避免前台体感像没反应。对中间不一定自然冒出结果的长步骤，优先用一个当前会话 3 分钟一次性检查来兜底这条规则，而不是只靠临场记忆。
7. **能力边界**
   - `main-supervisor-lite` 是兜底机制，不是万能保险。它能显著降低“主会话工作时像没反应”的概率，但不能保证覆盖所有故障。
   - 若监工已提前在位，且 Gateway、当前渠道投递、前端 WebSocket、以及至少一条可用模型调用链路仍正常，则它通常能在主会话本轮卡住时继续起作用，向前台补状态或在子任务完成后回报结果。
   - 若问题发生在**更底层的全局阻塞**，例如：Gateway 事件循环整体卡死、渠道投递本身失效、前端连接断开、对同一模型/同一路由的全局网络阻塞，监工也可能一起受影响，不能被视为独立于主系统之外的第二条绝对可靠链路。
   - 因此正确默认不是“有监工就一定万无一失”，而是：**监工前置 + 主会话短反馈 + 重活尽早卸载 + 异常先报告 + 媒体真实验收**，用这一整套组合降低风险，而不是把希望押在单一点上。

## 场景判定表

| 场景 | `main-supervisor-lite` | 普通任务分身 | 前台要求 | 备注 |
|---|---|---|---|---|
| 纯日常聊天 / 陪伴式闲聊 | 可为 0 | 不开 | 正常聊天即可 | 默认不强开监工 |
| 工作型任务，但不会明显阻塞前台 | 必须为 1 | 可不开 | 先短反馈，前台继续正常对话 | 监工待命，不必强开任务分身 |
| 工作型任务，且会明显阻塞前台 | 必须为 1 | 视需要开启 | 先短反馈，随后由监工兜底补进度 | 正常主线：监工 + 任务分身 |
| 已有普通任务分身在跑 | 必须为 1 | 可为 1..N（按任务而定） | 前台仍保持可回复 | 禁止出现“只有任务分身，没有监工” |
| 无后台任务持续约 2 分钟 | 可回到 0 | 应为 0 | 前台按正常聊天处理 | 监工关闭条件只看后台任务，不看是否还在聊天 |
| 任务空返回 / 异常结束 | 必须为 1 | 相关任务分身异常 | 监工必须先向前台报告 | 再开始检查、修复、必要时重开 |
| 3 分钟无可见产出 | 必须为 1 | 任意 | 监工必须主动补一句状态 | 不等于任务必须 3 分钟内完成 |
| Gateway/渠道/模型链路仍基本可用，但主会话本轮卡住 | 若已在位，通常可继续起作用 | 视任务而定 | 监工补状态或回报结果 | 这是监工真正擅长覆盖的场景 |
| Gateway 整体卡死 / 前端断开 / 渠道投递失效 / 同一路由模型全局阻塞 | 可能一起失效 | 可能一起失效 | 不能假设监工一定还能回报 | 这属于监工的能力边界 |
| 拿不准是不是纯日常聊天 | 默认按工作型任务处理 | 视是否阻塞而定 | 先短反馈 | 保守默认：宁可保留监工，不冒前台卡住风险 |
