...
- **默认执行顺序**：1）先给主会话一个短反馈；2）判断当前轮属于简单工作还是复杂工程，并切换监工服务到对应状态；3）若当前轮确实需要聊天插播/额外协作，再检查 `main-supervisor-lite` 是否已在位，若没有则补一个，且只保留一个；4）判断当前工作会不会阻塞前台，若会则立刻卸到后台任务分身；5）前台继续正常对话，只在关键节点主动插播；6）凡是媒体相关改动，都以主会话里的真实展示/播放成功为准；7）任务收尾时默认先关闭普通任务分身；若当前轮不再需要监工，则恢复 `auto + taskActive=false`，并在不再需要协作时收掉 `main-supervisor-lite`。
