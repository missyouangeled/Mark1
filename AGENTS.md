...
6. **异常模板**
   ...
   - **30 秒无可见产出**：这里的“可见产出”不是指任务必须 30 秒内完成，而是指用户前台在 30 秒内至少应该看到一次短反馈、进度、阶段结果或卡点说明。即使一切正常、只是还在等待，也必须由 `main-supervisor-lite` 主动补一句“仍在运行 / 还在等 / 下一次何时回报”，避免前台体感像没反应。
7. **能力边界**
...
| 30 秒无可见产出 | 必须为 1 | 任意 | 监工必须主动补一句状态 | 不等于任务必须 30 秒内完成 |
