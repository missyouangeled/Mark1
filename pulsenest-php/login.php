...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && site_setting_enabled('site.login_enabled', true)) {
    ...
    if ($identifier === '' || $password === '') {
        ...
        $stmt = db()->prepare('SELECT id, username, nickname, email, password_hash, avatar_path, bio, location, website_url, is_admin, is_active, role, created_at FROM pulsenest_users WHERE email = :id OR username = :id LIMIT 1');
