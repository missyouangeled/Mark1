...
$stmt = db()->prepare(
    'SELECT u.id, u.username, u.nickname, u.email, u.avatar_path, u.bio, u.location, u.website_url, u.created_at,
            COUNT(p.id) AS post_count
     FROM pulsenest_users u
     LEFT JOIN posts p ON p.user_id = u.id AND p.status = "published"
     WHERE u.id = :id
     GROUP BY u.id, u.username, u.nickname, u.email, u.avatar_path, u.bio, u.location, u.website_url, u.created_at
     LIMIT 1'
);
...
$profileAgeDays = $profile ? account_age_days($profile) : 0;
$creatorPresence = $profile ? creator_presence_summary($profile, [
    'post_count' => (int) ($profile['post_count'] ?? 0),
]) : null;
if ($profile) {
    ...
    $creatorPresence = creator_presence_summary($profile, [
        'post_count' => (int) ($profile['post_count'] ?? 0),
        'latest_post_at' => $profileStats['latest_post_at'] ?? null,
    ]);
      ...
      <section class="glass nebula-hero nebula-hero-split user-hero refined-hero refined-hero-profile">
        <div class="nebula-copy">
          ...
          <div class="chips" style="gap:6px; margin-top: 4px;">
            ...
            <?php if (!empty($profile['website_url'])): ?><span class="chip">链接：<?= e(profile_link_label($profile['website_url'])) ?></span><?php endif; ?>
            <?php if ((int) $profile['post_count'] > 0): ?><span class="chip">已开始公开创作</span><?php endif; ?>
            <?php if ($publicProfileSummary): ?><span class="chip">资料状态：<?= e($publicProfileSummary['tone']) ?></span><?php endif; ?>
            <span class="chip"><?= $profileAgeDays <= 7 ? '新加入成员' : '社区常驻成员' ?></span>
          </div>
        ...
        <aside class="profile-chip nebula-profile-chip user-profile-chip ops-side-panel">
          ...
          <div>
            ...
            <?php if (!empty($profile['location'])): ?><span>所在地：<?= e($profile['location']) ?></span><?php endif; ?>
            <?php if (!empty($profile['website_url'])): ?><span><a class="inline-link" href="<?= e($profile['website_url']) ?>" target="_blank" rel="noopener noreferrer"><?= e(profile_link_label($profile['website_url'])) ?></a></span><?php endif; ?>
            <span><?= (int) $profile['post_count'] > 0 ? '已开始公开创作' : '还在准备第一篇公开内容' ?></span>
          </div>
        </aside>
      </section>
        ...
        <aside class="right-col-stack">
            ...
            <?php if ($publicProfileSummary): ?>
              <div class="profile-progress-card compact-profile-progress">
                <div class="profile-progress-head">
                  <strong>公开资料完成度 <?= (int) $publicProfileSummary['percent'] ?>%</strong>
                  <span><?= e($publicProfileSummary['tone']) ?></span>
                </div>
                <div class="profile-progress-bar" aria-hidden="true"><span style="width: <?= (int) $publicProfileSummary['percent'] ?>%"></span></div>
                <div class="profile-progress-meta">
                  <span>已公开 <?= (int) $publicProfileSummary['done'] ?> / <?= (int) $publicProfileSummary['total'] ?> 层信息</span>
                  <span><?= !empty($publicProfileSummary['missing']) ? '仍有部分资料未公开。' : '这张公开名片已经比较完整。' ?></span>
                </div>
              </div>
            <?php endif; ?>
            <div class="detail-list">
              <div class="detail-row"><span>公开资料层次</span><strong><?= $publicProfileSummary ? (int) $publicProfileSummary['done'] . ' / ' . (int) $publicProfileSummary['total'] : '—' ?></strong></div>
              <div class="detail-row"><span>资料状态</span><strong><?= $publicProfileSummary ? e($publicProfileSummary['tone']) : '—' ?></strong></div>
              <div class="detail-row"><span>对外链接</span><strong><?= !empty($profile['website_url']) ? '已挂出' : '未公开' ?></strong></div>
              <div class="detail-row"><span>社区创作</span><strong><?= (int) $profile['post_count'] > 0 ? '已开始' : '还未开始' ?></strong></div>
              <div class="detail-row"><span>成员阶段</span><strong><?= $profileAgeDays <= 7 ? '新加入成员' : '稳定活跃档案' ?></strong></div>
              <div class="detail-row"><span>创作者状态</span><strong><?= $creatorPresence ? e($creatorPresence['label']) : '—' ?></strong></div>
            </div>
          <?php if ($creatorPresence): ?>
            <div class="profile-state-card compact-state-card">
              <div class="profile-state-head">
                <strong><?= e($creatorPresence['label']) ?></strong>
                <span><?= e($creatorPresence['meta']) ?></span>
              </div>
              <p><?= e($creatorPresence['note']) ?></p>
            </div>
          <?php endif; ?>
          <?php if ($publicProfileSummary && !empty($publicProfileSummary['missing'])): ?>
            <div class="presence-missing-line">这张名片还保留了部分留白：<?= e(implode(' / ', array_column(array_slice($publicProfileSummary['missing'], 0, 3), 'label'))) ?>。</div>
          <?php endif; ?>
          </section>
          ...
          <section class="glass panel-card surface-section">
            ...
            <div class="detail-list">
              ...
              <div class="detail-row"><span>所在地</span><strong><?= e($profile['location'] ?: '未公开') ?></strong></div>
              <div class="detail-row"><span>个人链接</span><strong><?php if (!empty($profile['website_url'])): ?><a class="inline-link" href="<?= e($profile['website_url']) ?>" target="_blank" rel="noopener noreferrer"><?= e(profile_link_label($profile['website_url'])) ?></a><?php else: ?>未公开<?php endif; ?></strong></div>
          ...
          <section class="glass panel-card surface-section">
            ...
            <div class="quick-links curated-stack">
              ...
              <?php if (!empty($profile['website_url'])): ?>
                <a class="quick-link" href="<?= e($profile['website_url']) ?>" target="_blank" rel="noopener noreferrer"><strong>访问作者链接</strong><span>查看 <?= e($profile['nickname']) ?> 公开挂出的个人主页或作品集。</span></a>
              <?php endif; ?>
              <?php if ($currentViewer && (int) $currentViewer['id'] === (int) $profile['id']): ?>
                <a class="quick-link" href="/account.php#profile-studio"><strong>编辑我的资料</strong><span>维护头像、简介和个人主页信息。</span></a>
              <?php endif; ?>
            </div>
          </section>
        </aside>
