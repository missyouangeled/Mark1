...
当子任务的目标执行环境与当前机器不同（例如在公司 Linux 机上为掌机 Windows 部署任务），按以下协议处理：

### 原则
分身只能操作其所在机器的本地文件系统。不能假设可以 `exec` 或 `write` 到远程机器。

### 执行路径
1. **方案先行**：先在当前机器起草方案/脚本/配置，明确目标机器的系统差异（OS、包管理器、服务管理方式）
2. **交付为文件**：产出目标机器需要的文件（`.cmd`/`.ps1` 脚本、配置文件），push 到共享 Git 仓库
3. **远程执行由目标机器的 OpenClaw 自己完成**：在目标机器上通过 `git pull` 拉取更新后，再由该机器的 OpenClaw 执行部署
4. **验收同样走回传**：目标机器上的 OpenClaw 执行验收命令后，将结果 commit 回 repo 或通过聊天回报

### 注册/标记
- 跨机器任务应在 `PLANS.md` 中明确标注「跨机器：源机器 → 目标机器」
- 部署脚本头部标明 `适用机器：xxx（OS）`，让目标机器的 OpenClaw 能自动识别
- 如果任务需要"等掌机上线后继续"，使用 cron job 设定提醒，而不是无限期占用分身

### 边界
- 此协议只处理"同一个 workspace repo 内的跨机器协作"
- 不处理 SSH 直连远程执行（属于机主机操作，按 AGENTS.md 权限规则走）
