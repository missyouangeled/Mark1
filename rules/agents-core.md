...
1.5 **涉及系统工具 / provider / capability 时，先确认当前版本支持不支持**（点点 2026-08-06 定为硬规矩，CASE-20260806-016）
   - 顺序：官方文档 `docs/providers/` → 源码内置适配器表 → 才动配置
   - ⚠️ **`config validate` 通过 ≠ 功能可用**：它只校验配置格式合法，不校验能力是否存在
   - ⚠️ **API 直调成功 ≠ 工具路径可用**：curl 走厂商 API，工具走 OpenClaw 适配器层，两条不同链路
   - 真正的验收是：**实际调用一次工具成功**
2. 再查冲突与边界
...
> ⚠️ 这是铁律：任何检查/修复/验证任务，最终结果不能有任何黄色警告（warn/yellow）。只要不是全绿，就再来一遍：审查 → 联网查询 → 修改 → 烟测，直到全绿为止。

- 黄色警告不是「不耽误使用」，是「还没做完」
- 哪怕只剩一条 warn，也必须继续排查修复
- 修复后要把问题和解决方法记录到对应位置：
  - 升级相关 → `docs/通用-OpenClaw-升级记录.md`
  - 补丁相关 → `docs/通用-OpenClaw-补丁变更流水.md`
  - 一般错误 → `.learnings/ERRORS.md`
  - 新发现的能力缺陷 → `.learnings/FEATURE_REQUESTS.md`
  - 不确定归哪 → `.learnings/INBOX.md`
- 如果是全新工程/无对应记录位置，最终汇报中明确告诉用户「已修复但暂无对应记录位置」
